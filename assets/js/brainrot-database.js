...
let state = { all: [], filtered: [], selectedId: null, metadata: null };
...
function saveFilters(){ localStorage.setItem(FILTER_KEY, JSON.stringify(getFilters())); }
...
function syncQuery(){
  ...
  const f = getFilters();
  Object.entries(f).forEach(([k,v])=>{ if(v && v !== 'All' && !(k==='sort' && v==='name-asc')) u.searchParams.set(k,v); else u.searchParams.delete(k); });
  if(state.selectedId) u.searchParams.set('brainrot', state.selectedId); else u.searchParams.delete('brainrot');
...
function applyFilters(){
  ...
  if(f.q){
    ...
    list = list.filter(x => x.name.toLowerCase().includes(q) || (x.aliases || []).some(a => a.toLowerCase().includes(q)));
  }
  ...
  if(f.sort === 'rarity') list.sort((a,b)=>rarityOrder.indexOf(a.rarity)-rarityOrder.indexOf(b.rarity) || a.name.localeCompare(b.name));
  state.filtered = list;
  render();
  saveFilters();
  syncQuery();
}
function row(label,value){ return value ? `<p><strong>${label}:</strong> ${value}</p>` : ''; }
...
function openModal(id){
  ...
  el.modalBody.innerHTML = [
    ...
    item.sourceUrls?.length ? `<p><strong>Sources:</strong> ${item.sourceUrls.map(u=>`<a href="${u}" target="_blank" rel="noopener">link</a>`).join(' · ')}</p>` : ''
  ].join('');
  el.detailModal.showModal();
  syncQuery();
}
function closeModal(){ state.selectedId = null; el.detailModal.close(); syncQuery(); }
...
async function init(){
  try{
    ...
    const rarities = [...new Set(state.all.map(x=>x.rarity || 'Unknown'))].sort((a,b)=>rarityOrder.indexOf(a)-rarityOrder.indexOf(b));
...
document.addEventListener('input', (e)=>{ if([el.searchInput].includes(e.target)) applyFilters(); });
document.addEventListener('change', (e)=>{ if([el.rarityFilter, el.spawnFilter, el.sortSelect].includes(e.target)) applyFilters(); });
el.clearFiltersBtn.addEventListener('click', ()=>{ el.searchInput.value=''; el.rarityFilter.value='All'; el.spawnFilter.value='All'; el.sortSelect.value='name-asc'; applyFilters(); });
...
el.copyItemLinkBtn.addEventListener('click', async ()=>{ const u = new URL(location.href); if(state.selectedId) u.searchParams.set('brainrot', state.selectedId); await navigator.clipboard.writeText(u.toString()); alert('Deep-link copied ✅'); });
el.copyViewBtn.addEventListener('click', async ()=>{ await navigator.clipboard.writeText(location.href); alert('View URL copied ✅'); });
